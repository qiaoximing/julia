...
function state_init(net::Net)
    ...
    # options inlucde all expansions and no merge
    options = [Option([dg]) for dg in dgroups]
    return State(trees, lroots, rroots, expans, lslots, rslots, options)
end
...
function update_io!(dgroup::Group{Distr}, result::Int, state::State, net::Net)
    ...
    deletefirst!(x->x.dgroups[1]===dgroup, state.options)
end
...
function link_io!(node::Tree, d1, d2, state::State)
    if d1 isa Distr && d2 isa Distr
        ...
        if g1 !== g2
            ...
            deletefirst!(x->x.dgroups[1]===g2, state.options)
        end
...
# create new node and return it, update state
function create_node!(result::Int, state::State, net::Net)
    node, distrs = tree_init(result, net.size) 
    dgroups = [group_init(d) for d in distrs if sum(d.prob)>0]
    # state update
    push!(state.trees, node)
    push!(state.expans, dgroups)
    # add roots (might be nothing)
    push!(state.lroots, getfirst(x->x.factors[1]===tree.lparent, dgroups))
    push!(state.rroots, getfirst(x->x.factors[1]===tree.rparent, dgroups))
    # allow overlapping l/r slots for unobserved nodes
    push!(state.lslots, [[dg for dg in dgroups if dg.factors[1].targ in (Left, Right)]])
    push!(state.rslots, [[dg for dg in dgroups if dg.factors[1].targ in (Left, Right)]])
    # options only include expansions
    append!(state.options, [Option([dg]) for dg in dgroups])
    return node
end
...
function connect!(node1::Tree, node2::Tree, axis::Axes, state::State, net::Net, observed::Bool=true)
    ...
    deletefirst!(x->(length(x.dgroups)==1 &&
                     x.dgroups[1].prod.node===node1 && 
                     x.dgroups[1].prod.targ==axis), state.options)
    deleteall!(x->(length(x.dgroups)==1 &&
                   x.dgroups[1].prod.node===node2 && 
                   x.dgroups[1].prod.targ==Node), state.options)
    # remove roots
    deleteat!(state.lroots, t2_idx)
    deleteat!(state.rroots, t2_idx)
    # remove merge options with t2 as root
    ...
    # remove merge options with removed slots
    # add new options from new slots and new adjacencies
    # remove trees
...
function run_option!(option::Option, result::Int, state::State, net::Net)
    ...
    if length(option.dgroups == 1)
        ...
        elseif targ in (Input, Output)
            ...
            # create new node and add it to state.trees
            new_node = create_node!(result, state, net)
...
function parse(net, data)
    ...
    for step in 1:max_steps
        if length(state.trees) == 1 && length(state.options) == 0
            success = true
            break 
        end
        option_idx = sample(state.options) # select an option
        option = state.options[option_idx]
        result = sample(option.prod) # sample a result from the option
        run_option!(option, result, state, net)
        # deleteat!(state.options, option_idx)
        println("step $step:")
        println(state)
    end
